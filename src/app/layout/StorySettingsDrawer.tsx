          ...
          <Section title="AI Settings">
            <form className="space-y-3" onSubmit={saveAI}>
              ...
              <label className="block space-y-2 text-sm text-ink-muted">
                <FieldLabel label="Audiobook performance" help="Choose one narrator for the whole story, or unique character voices for dialogue." labelClassName="text-xs text-ink-muted" />
                <select
                  className="w-full rounded-[8px] border border-divider bg-panel-muted/50 px-3 py-2.5 text-sm text-ink"
                  value={performanceMode}
                  onChange={(e) => setPerformanceMode(e.target.value as AudiobookPerformanceMode)}
                >
                  <option value="single_narrator">Single narrator — one voice for everything</option>
                  <option value="radio_drama">Character voices — unique voices for dialogue</option>
                </select>
              </label>
