...
interface StoryEngineContextValue {
  ...
  sendChatMessage: (storyId: string, content: string, opts?: { directorIntentOverride?: DirectorIntent; skipAssistantResponse?: boolean; signal?: AbortSignal; guidedGenerationInternal?: boolean; directorStagingNote?: string; guidedDirectedScene?: boolean; guidedChapterContext?: { overallDirection?: string; chapterOverview?: string; chapterLabel?: string; sceneOverview?: string; continuityNotes?: string; previousChapterContext?: string }; onChunk?: (chunk: string) => void; onChunkReset?: () => void; onGenerationAttempt?: (attempt: number, maxAttempts: number) => void }) => Promise<{ message: StoryMessage | null }>;
  ...
  const value = useMemo<StoryEngineContextValue>(() => {
    ...
    return {
      ...
      async createSequel(parentStoryId, title) {
        ...
        if (parentState) {
          const parentRuntimeState = safeParseStoryStateData(parentState.stateJson);
          await repository.saveStoryState({
            ...parentState,
            id: `story-state:${storyId}`,
            storyId,
            stateJson: JSON.stringify(parentRuntimeState),
            updatedAt: now,
          });
        }
      ...
      async regenerateLastAssistantMessage(storyId, opts) {
        ...
        const context = injectSceneSpeakerRegistry(
          buildStoryChatContext({
            ...
            latestUserMessageSpeakerName: previousMessage.speakerName,
        ...
        const validateCandidate = (initialText: string) =>
          resolveStreamedAssistantTranscript({
            ...
            playerPronouns: playerIdentity.pronouns,
            playerAliases: Array.from(new Set([...normalizePlayerCharacterAliases(playerCharacter.aliases), ...(previousMessage.speakerType === "player" && previousMessage.speakerName?.trim() ? [previousMessage.speakerName.trim()] : [])])),
        ...
        if (!shouldSkipAssistantReply) {
          ...
          const context = injectSceneSpeakerRegistry(
            buildStoryChatContext({
              ...
              latestUserMessageSpeakerName: userMessage.speakerName,
              allowDirectedPlayerControl,
              directorIntent: userMessage.directorIntent ?? null,
          ...
          const validateCandidate = (initialText: string) =>
            resolveStreamedAssistantTranscript({
              ...
              playerSceneName: playerIdentity.sceneName,
              playerPronouns: playerIdentity.pronouns,
              playerAliases: Array.from(new Set([...normalizePlayerCharacterAliases(playerCharacter.aliases), ...(userMessage.speakerType === "player" && userMessage.speakerName?.trim() ? [userMessage.speakerName.trim()] : [])])),
              characterGenders: streamCharacterGenders,
              allowDirectedPlayerControl,
              speakerRegistry,
              resolvedParticipants,
