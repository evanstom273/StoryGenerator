...
export function StoryAudioPlayerBar({ className }: { className?: string }) {
	...
	const visible =
		status === "loading" || status === "ready" || status === "playing" || status === "error";
	...
	useEffect(() => {
		...
		const tick = () => {
			const seconds = Math.max(0, Math.floor((Date.now() - loadingDetail.startedAtMs) / 1000));
		...
		>
			<div className="mx-auto flex max-w-3xl flex-col gap-2">
				...
				<input
					...
					disabled={isLoading || status === "error" || durationSec <= 0}
