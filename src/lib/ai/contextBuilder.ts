...
export function buildStoryChatContext({
  ...
  const sceneGuidance = normalizeWhitespace(
    [
      ...
      "- Each time a character speaks, give them real substance before switching to another speaker. A character's block should feel like a genuine contribution to the scene — multiple sentences of dialogue, a reaction, a follow-up, a thought. A single nod plus one word, or one short sentence, is not enough. Aim for 3–5 sentences per speaker turn. Short back-and-forth exchanges ('Yes.' / 'No.' / 'Sure.') should be avoided entirely — fold that rhythm into fuller, more expressive turns.",
