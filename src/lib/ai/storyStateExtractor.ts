...
export function buildStoryStateExtractionPrompt({
  ...
  const system = normalizeWhitespace(
    [
      ...
      "- When the player's scene name differs from their legal/canonical name and the true identity has not been revealed, create characters and indexes.characters entries under the scene name (e.g. Mark Owen), not under the legal/canonical name. Keep canonicalName on that entry for internal continuity only.",
      "- Before an identity reveal, treat the scene identity and canonical identity as separate tracked characters in reader-facing index UI: show only the scene name (witness/NPC-facing), never the hidden legal/canonical name in Character Status, Characters, or relationship labels.",
      ...
      "- For indexes.relationships: create an entry for the player character paired with each established character ONLY after the player has appeared or been mentioned in in-fiction transcript. Also create entries between NPCs who have direct notable interaction.",
