...
function computeBackupSignature(backup: any) {
  ...
  const maxTimestamp = (list: any[], field: string) => {
    let max = 0;
  ...
  const maxes = {
    ...
    universeImportsImportedAt: maxTimestamp(Array.isArray(data.universeImports) ? data.universeImports : [], "importedAt"),
    storySummariesGeneratedAt: maxTimestamp(Array.isArray(data.storySummaries) ? data.storySummaries : [], "generatedAt"),
  };
...
export async function runAndroidAutoBackupIfNeeded(repository: StoryEngineRepository) {
  ...
  try {
    ...
    const files = (listing.files ?? []).map((entry: any) => String(entry.name ?? entry)).filter(Boolean);
    const backups = files
