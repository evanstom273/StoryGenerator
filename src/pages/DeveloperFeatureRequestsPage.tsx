...
export function DeveloperFeatureRequestsPage() {
  const { developerFeatureRequests, deleteDeveloperFeatureRequest } = useStoryEngine();

  async function handleDelete(id: string) {
    const confirmed = window.confirm(`Delete ${id}?`);
    if (!confirmed) return;
    await deleteDeveloperFeatureRequest(id);
  }
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        description="Capture new ideas and future improvements separately from bugs."
        actions={
          <div className="flex flex-wrap gap-3">
            <Link to="/developer-notes" className={buttonClasses({ variant: "secondary" })}>
              Overview
            </Link>
            <Link to="/developer-notes/features/new" className={buttonClasses()}>
              New Feature
            </Link>
          </div>
        }
      ...
      {developerFeatureRequests.length ? (
            ...
            >
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                ...
                <div className="flex shrink-0 flex-wrap gap-2">
                  <Link
                    to={`/developer-notes/features/${feature.id}/edit`}
        ...
        <EmptyState
          ...
          description="Capture the next improvement idea the moment it comes up during testing."
          action={
            <Link to="/developer-notes/features/new" className={buttonClasses()}>
              Create Feature
            </Link>
